...
- The selected release fixture has no usable JetKVM ATX motherboard leads. The user approved a candidate-bound `atx_unavailable` hardware-validation profile for v0.1: physical ATX switching is excluded from release claims, while the ten-tool API and controlled/fake/unit ATX contract remain in scope. This is not a runtime waiver; the frozen candidate, hardware evidence, validator, and release notes must all bind and disclose the profile.
...
8. **Live hardware:** manual/nightly and mandatory for release, serialized under the release lease. A frozen `full` profile requires every live step. A frozen `atx_unavailable` profile may exclude only the canonical step set that physically depends on live `jetkvm_power_control` wiring and must produce machine-validated exception evidence. Every live story still runs; mixed stories continue with every remaining safe step and their complete baseline/restore sequence.
...
The frozen candidate records exactly one hardware-validation profile. `full` is the default and preserves the zero-skip contract. `atx_unavailable` requires an explicit freeze-time acknowledgement that the selected fixture has no usable ATX leads. Under that profile the runner does not perform the ATX reset preflight or any physical power mutation, computes the excluded step IDs from the canonical execution plan rather than a caller-supplied list, and runs every live story plus every remaining step in manifest order. The immutable result is `pass_with_exception`, names the exact excluded `(story_id, step_id)` pairs and reason, and still requires every executed step, story baseline, and restore to pass. The validator rejects any runtime/profile mismatch, extra exclusion, missing non-ATX step, missing story, failed restore, unknown outcome, or undisclosed exception.
...
Phase 6 freezes PR-head commit `C`, tree `G`, one allowlisted npm tarball `T`, and one hardware-validation profile `P`. The hardware lease covers deployment/readiness, every live story, every profile-required step, every per-story restore, final status, evidence flush, and cleanup. Live evidence must identify `C/G/T/P`; it may not be reused from another tree, artifact, or profile.

The external immutable manifest is a PR/CI artifact and later a GitHub release asset, never self-referential content inside `G`. It records spec/schema/story hashes, candidate commit/tree, tarball/lockfile/package-tree hashes, Node/browser/device versions and hashes, profile `P`, every live-story outcome, all profile-required step outcomes, any exact machine-derived ATX step exclusion, operation/request/observation/generation IDs, sanitized timing/count/hash evidence, restore results, and artifact hashes. `full` requires zero exclusions and `result: pass`; `atx_unavailable` requires only the exact ATX-dependent step exclusion and `result: pass_with_exception`.

Merge must preserve `C` as an ancestor and produce the reviewed tree `G`. Conflict resolution or any source/package/lockfile/profile/tree change creates a new candidate and reruns affected gates and profile-required live steps. The release tag points to the verified candidate. The GitHub release contains exact `T`, checksum, schemas, compatibility/setup/security/rollback guidance, immutable manifest, and any candidate-bound hardware exception.

Finally, a clean directory downloads the release assets, verifies checksums, installs `T`, initializes and lists exactly the ten tools over stdio and loopback legacy SSE, exercises manifest smoke stories, and runs the offline doctor with zero device signaling. Release is blocked unless this exact-artifact verification and final hardware restoration both pass. An `atx_unavailable` release must prominently state that physical ATX switching was not validated on the selected fixture and must not convert controlled serial/acknowledgement evidence into a host-state claim.
