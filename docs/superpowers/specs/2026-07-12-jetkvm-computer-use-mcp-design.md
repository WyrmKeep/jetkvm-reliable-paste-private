...
8. **Live hardware:** manual/nightly and mandatory for release, serialized under the release lease. A frozen `full` profile requires every live story. A frozen `atx_unavailable` profile may omit only the canonical story set derived from live `jetkvm_power_control` actions and must produce machine-validated exception evidence; every other live story remains mandatory.
...
The frozen candidate records exactly one hardware-validation profile. `full` is the default and preserves the zero-skip contract. `atx_unavailable` requires an explicit freeze-time acknowledgement that the selected fixture has no usable ATX leads. Under that profile the runner does not perform the ATX reset preflight or any physical power mutation, computes the excluded story IDs from the canonical execution plan rather than a caller-supplied list, and runs every remaining live story in manifest order. The immutable result is `pass_with_exception`, names the exact excluded IDs and reason, and still requires every executed story and restore to pass. The validator rejects any runtime/profile mismatch, extra exclusion, missing non-ATX story, failed restore, unknown outcome, or undisclosed exception.
...
Phase 6 freezes PR-head commit `C`, tree `G`, one allowlisted npm tarball `T`, and one hardware-validation profile `P`. The hardware lease covers deployment/readiness, every profile-required live story, every per-story restore, final status, evidence flush, and cleanup. Live evidence must identify `C/G/T/P`; it may not be reused from another tree, artifact, or profile.

The external immutable manifest is a PR/CI artifact and later a GitHub release asset, never self-referential content inside `G`. It records spec/schema/story hashes, candidate commit/tree, tarball/lockfile/package-tree hashes, Node/browser/device versions and hashes, profile `P`, all profile-required story outcomes, any exact machine-derived ATX exclusion, operation/request/observation/generation IDs, sanitized timing/count/hash evidence, restore results, and artifact hashes. `full` requires zero skips and `result: pass`; `atx_unavailable` requires only the exact ATX exclusion and `result: pass_with_exception`.

Merge must preserve `C` as an ancestor and produce the reviewed tree `G`. Conflict resolution or any source/package/lockfile/profile/tree change creates a new candidate and reruns affected gates and profile-required live stories. The release tag points to the verified candidate. The GitHub release contains exact `T`, checksum, schemas, compatibility/setup/security/rollback guidance, immutable manifest, and any candidate-bound hardware exception.
