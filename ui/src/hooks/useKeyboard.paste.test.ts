...
const fixture = vi.hoisted(() => ({
  ...
  rtc: {
    rpcHidChannel: null as unknown,
    rpcHidProtocolVersion: 1 as number | null,
...
vi.mock("@/hooks/stores", () => ({
  ...
  useHidStore: Object.assign(
    () => ({
      ...fixture.hid,
      setKeysDownState: vi.fn(),
      setKeyboardLedState: vi.fn(),
      setPasteModeEnabled: vi.fn(),
    }),
    {
...
describe("paste input safety", () => {
  it("does not silently use legacy typing before HID readiness", async () => {
    ...
    await expect(
      keyboard.executePasteText("a", options(new AbortController().signal)),
    ).rejects.toThrow("requires ready HID RPC");
  ...
  it("rejects a macro refused by the transport", async () => {
    ...
    const { default: useKeyboard } = await import("./useKeyboard");
    await expect(
      useKeyboard().executePasteText("a", options(new AbortController().signal)),
  ...
  it("rejects channel loss during a bufferedAmount wait", async () => {
    ...
    await expect(observed).resolves.toMatchObject({
      message: "Paste HID channel closed during backpressure",
    });
