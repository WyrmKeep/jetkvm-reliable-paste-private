    ...
    for _, gap := range []time.Duration{2, 20, 45} {
        for _, stall := range []time.Duration{0, 2, 10, 50, 250} {
            t.Run(fmt.Sprintf("gap%d-stall%d", gap, stall), func(t *testing.T) {
                ...
                for i := range delays { delays[i] *= time.Millisecond }
                now := time.Unix(0, 0)
                ...
                err := run(context.Background(), delays, func(i int) error {
                    starts[i] = now
                    if i == 0 || i == 2 { now = now.Add(stall * time.Millisecond) }
                    completed[i] = now
                    return nil
                    ...
                    now = deadline
                    if waits == 1 { now = now.Add(stall * time.Millisecond) }
                    waits++
                    return nil
                })
...
func TestCancellationAndWriteFailureStopTheSequence(t *testing.T) {
    for _, scenario := range []string{"before", "write", "wait"} {
        t.Run(scenario, func(t *testing.T) {
            ...
            if scenario == "write" { want = sentinel }
            if !errors.Is(err, want) { t.Fatalf("got %v, want %v", err, want) }
            if scenario == "before" && writes != 0 { t.Fatal("wrote after cancellation") }
            if writes > 1 || waits > 1 { t.Fatal("continued after failure") }
        })
    }
}
...
func TestNegativeDelayFailsBeforeAnyWrite(t *testing.T) {
    ...
    if err == nil || writes != 0 { t.Fatalf("err=%v writes=%d", err, writes) }
}
...
func TestFinalReleaseSurvivesBatchBoundary(t *testing.T) {
    ...
    if err := run(context.Background(), []time.Duration{5 * time.Millisecond, 20 * time.Millisecond}, write, func() time.Time { return now }, wait); err != nil { t.Fatal(err) }
    if now.Sub(lastWrite) != 20*time.Millisecond { t.Fatal("missing final release delay") }
    nextStart := now
    if err := run(context.Background(), []time.Duration{5 * time.Millisecond}, write, func() time.Time { return now }, wait); err != nil { t.Fatal(err) }
    if lastWrite != nextStart { t.Fatal("next batch timing changed") }
}
